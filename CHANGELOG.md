...
## [1.2.0] - 15.09.2025
### Добавлено
- **Полная система метрик и статистики**: новый флаг `--metric` для детального анализа использования пароля
- **Отслеживание статистики профиля**: количество сгенерированных паролей, даты первого и последнего использования
- **Реальный расчет энтропии**: динамическое вычисление энтропии на основе длины пароля и размера алфавита по формуле E = L × log₂(A)
- **Анализ распределения силы паролей**: динамическое вычисление процентов распределения на основе криптографической силы Argon2
- **Метрики производительности**: измерение и отображение реального времени генерации паролей
- **Системная информация**: отображение платформы, версии, параметров Argon2
- Поля `TotalGenerationTime` и `AverageGenerationTime` в структуру `ProfileStatistics`
- Функция `calculateAlphabetSize()` для определения размера алфавита:
  - alphanumeric: 62 символа (A-Z, a-z, 0-9)
  - alphanumeric_symbols: 94 символа (+ спецсимволы)
  - symbols_only: 32 символа (только спецсимволы)
- Отображение счетчика паролей в заголовке CLI: `[профиль] [количество]`

### Изменено
- **Функция `IncrementPasswordCount()`**: теперь принимает параметр времени генерации в миллисекундах
- Измерение времени генерации паролей с использованием `time.Now()` и `time.Since()`
- Обновлены тесты в `config_test.go` для проверки функциональности измерения времени
- Добавлены тесты в `root_test.go` и `messages_test.go` для новой системы метрик

### Техническое
- Все метрики основаны на реальных вычислениях и измерениях
- 100% покрытие тестами новой функциональности метрик
- Обратная совместимость с существующими конфигурационными файлами
- Автоматическое создание полей статистики при первом запуске
- Корректная обработка случаев с нулевым количеством паролей
...
[Unreleased]: https://github.com/MaksymLeiber/pgen/compare/v1.2.0...HEAD
[1.2.0]: https://github.com/MaksymLeiber/pgen/compare/v1.1.0...v1.2.0
